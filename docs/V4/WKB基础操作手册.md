...
<font size=2>组织架构配置 >人员导入 >权限配置 >项目配置 >办公资产配置 >社群管理 </font>

**1.2运营场景再现**

<font size=2>初次进入系统时，面对“空空如也”的系统，作为运营的应该怎样将系统配置起来呢？</font>

<font size=2>首先需要在`公司模式`下`公司配置`里点击`部门管理`将公司的组织架构创建进系统，在组织架构创建成功后，可以根据实际情况建立人员权限【系统会自动生成一部分默认权限组】如果还想细致划分可以在`角色管理`中配置。</font>![20](pic/20.png)

![21](pic/21.png)

<font size=2>当把部门和权限都配置好后就可以着手创建新的项目了，点击`项目管理`找到`新增项目`，这时会看到页面右侧会出现填写项目信息的页面，根据提示将项目数据填写明确【2.3.2中有提到的注意事项需要注意】</font>![22](pic/22.png)

<font size=2>项目创建好了之后就可以将公司的员工导入进系统中了，系统支持批量导入【按照模板格式，系统会进行校验】这里有一点需要注意，就像之前功能介绍中提到的系统中的权限分为公司角色和项目角色，在添加人员的时候一定要把两个类型的权限都赋予上</font>![23](pic/23.png)

<font size=2>接下来可以进入项目模式【点击右上角的`进入项目`]进行办公资产和社区管理配置，找到`办公资产`点击`房源维护`系统支持批量导入房源，操作的细节可以参考功能概要2.3.5；在添加房源信息的时候需要注意月租金有两种设定方式，一种是直接设置好固定金额，另外一种是设置好每平方米单价，系统会算出月租金，如果要想客户收取物业费并想通过系统来记录整理的话，不要忘了设置物业单价</font>![24](pic/24.png)

<font size=2>在`办公资产`中还有一个按钮是`场馆维护`，如果的项目上有路演厅、会议室、露台等场馆并且想要对内对外开放的话，可以在这里把它录入进系统中</font>![25](pic/25.png)

<font size=2>经过上述操作，系统就配置了起来，也为其他部门（招商、财务）建立起了基础操作必备的条件。</font>
...
<font size=2>上述基础信息搭建录入完成后，就可以进行系统中招商环节了，首先需要做的是配置招商团队，在`公司模式`下，点击`招商管理`，找到`招商配置`点击添加团队，讲团队信息录入系统</font>![26](pic/26.png)

<font size=2>建立好团队后，点击团队右侧成员按钮，点击添加就可以将已经导入过的公司成员加入进创建好的销售团队里了，同时可以在这里设置团队主管</font>![27](pic/27.png)

<font size=2>当创建好招商团队之后就可以组织团队成员进行客户的录入了，客户录入支持批量单入，当然在这个环节客户信息录入的越详尽后期得到的汇总数据就越丰富，在招商管理中设置有`意向管理`，在这个模块中销售代表可以将客户线索详细地记录在系统中</font>![28](pic/28.png)

<font size=2>当跟进的客户最终决定成单签署合同时，就到了创建新订单的时候了，不过在这之前需要确定是否已经将合同模板录入到了进系统中，那么如何添加合同模板呢？点击`招商管理`里面的`合同管理`点击`添加模板`将模板信息导入进富文本编辑器中保存就好了。有了合同模板再添加新的订单就好了【创建订单的操作细节可以参考2.3.8】</font>
...
<font size=2>和招商配置一样，需要先配置的财务团队，不同的是需要为每个团队配置“财务主体”，这个财务主体可以是相同的可以是不同的，简单理解就是设置对公账号</font>![29](pic/29.png)

<font size=2>在系统中需要明确费用和账单的关系，系统中租金和物业费【周期性费用】会自动生成，同一个客户的不同费用可以组合成一个账单，账单可以删除但是账单的费用还是会在费用管理中。在设置好财务团队并理解了费用和账单的定义之后，就可以在系统中进行调整费用、生成账单、确认收款、押金核销与退款等所有操作</font>
